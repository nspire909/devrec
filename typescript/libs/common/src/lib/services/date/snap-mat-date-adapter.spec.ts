...
//NOTE these test are almost identical to snap-iso-string-mat-date-adapter.spec.ts, if you one
//  the same change may be needed in the other.
describe('SnapMatDateAdapter', () => {
  describe('format and parse patterns', () => {
    let adapter: SnapMatDateAdapter;
    ...
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        providers: [
          ...
          { provide: DateAdapter, useClass: SnapMatDateAdapter },
        ],
      }).compileComponents();
    }));

    beforeEach(
      inject([DateAdapter], (d: SnapMatDateAdapter) => {
        adapter = d;
      })
    );
    ...
    describe('YYYY-MM-DD', () => {
      it('should format correctly', () => {
        const date = adapter.format(new Date(300, FEB, 3), 'YYYY-MM-DD');
        expect(date).toBe('0300-02-03');
      });
      it('should parse correctly', () => {
        const dates: [string, Date | null][] = [
          ['300-01-01', null],
          ['1977-07-21', new Date(1977, JUL, 21)],
          ['77-07-01', new Date(2077, JUL, 1)],
          ['77-7-21', new Date(2077, JUL, 21)],
          ['7-7-21', null],
          ['07-7-1', new Date(2007, JUL, 1)],
        ];
    ...
    describe('MM-DD-YYYY', () => {
      it('should format correctly', () => {
        const date = adapter.format(new Date(300, FEB, 3), 'MM-DD-YYYY');
        expect(date).toBe('02-03-0300');
      });
      it('should parse correctly', () => {
        const dates: [string, Date | null][] = [
          ['01-01-300', null],
          ['07-21-1977', new Date(1977, JUL, 21)],
          ['07-01-77', new Date(2077, JUL, 1)],
          ['7-21-77', new Date(2077, JUL, 21)],
          ['7-21-7', null],
          ['7-1-07', new Date(2007, JUL, 1)],
        ];
        dates.forEach(([dateString, expectedDate]) => {
    ...
    describe('DD-MMM-YYYY', () => {
      it('should format correctly', () => {
        const date = adapter.format(new Date(300, FEB, 3), 'DD-MMM-YYYY');
        expect(date).toBe('03-FEB-0300');
      });
      it('should parse correctly', () => {
        const dates: [string, Date | null][] = [
          ['01-JAN-300', null],
          ['21-JUL-1977', new Date(1977, JUL, 21)],
          ['01-JUL-77', new Date(2077, JUL, 1)],
          ['21-JUL-77', new Date(2077, JUL, 21)],
          ['21-JUL-7', null],
          ['1-JUL-07', new Date(2007, JUL, 1)],
        ];
        dates.forEach(([dateString, expectedDate]) => {
  ...
  describe('should pass the same test as NativeDateAdapter', () => {
    ...
    beforeEach(
      inject([DateAdapter], (dateAdapter: SnapMatDateAdapter) => {
        adapter = dateAdapter;
    ...
    it('should parse string', () => {
      expect(adapter.parse('1/1/2017', 'MM-DD-YYYY')).toEqual(
        new Date(2017, JAN, 1)
      );
    });
    ...
    it('should parse number', () => {
      ...
      expect(adapter.parse(timestamp, 'MM-DD-YYYY')).toEqual(
        new Date(timestamp)
      );
    });
    ...
    it('should parse Date', () => {
      ...
      expect(adapter.parse(date, 'MM-DD-YYYY')).not.toBe(date);
    });
