...
export interface ActionMetadata<TState> {
  actionName: string;
  methodName: string;
  action: (state: TState, payload: IAction) => TState;
}
...
export function getActionMetadataEntry<TState, TP>(
  ...
  throw Error(
    'Action function is not registered, it must be decorated with @FeatureAction().'
  );
}
 ...
 */
export function getActionMetadataEntries<TState>(
  sourceProto: FeatureActions<TState>
): ActionMetadata<TState>[] {
  const reducerMeta = getReducerMetadataEntry(sourceProto);
  const list = ((sourceProto as any).constructor[METADATA_KEY] ||
    []) as ActionMetadata<TState>[];
  // class decorators run after the method decorators
  list.forEach(
    action =>
      (action.actionName = `[${reducerMeta.prefix}] ${action.methodName}`)
  );
...
export function getReducerMetadataEntry(sourceProto: any): ReducerMetadata {
  ...
  if (!meta || !meta.prefix) {
    throw new Error(
      `Reducer class "${
        sourceProto.constructor.name
      }" is missing a @FeatureReducer decorator`
    );
  }
  return meta;
}
  ...
  return (ctor: Function) => {
    ...
    if (reducerCache[prefix]) {
      ...
      throw new Error(
        `Reducer class "${
          ctor.name
        }", Value: '${prefix}' is not unique. Use a different value in the @FeatureReducer decorator.`
      ); // tslint-
    }
    ...
    Object.defineProperty(ctor.prototype, METADATA_CLASS_KEY, {
      value: metadata
    });
  };
}
...
export function FeatureAction<TState>() {
   ...
   **/
  return <
    T extends FeatureActions<TState>,
    R extends (state: TState, payload: any) => TState
  >( // tslint:disable-line
    target: T,
    name: string | symbol,
    ...
    if (descriptor.value === undefined) {
      ...
      throw Error(
        'FeatureAction "' + name.toString() + '" function cannot be undefined!?'
      );
    }
    // class decorators run after method decorators
