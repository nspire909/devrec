    ...
    sort$.pipe(
      map(sort => {
        ...
        if (!sortFn) {
          ...
          //By default assume sort.active is a property name, an sort using the default sort
          //  uses < and >.
          // DEFAULT SORT: the table column matColumnDef value must match the property name of the sorted row object !!
          sortFn = (a: U, b: U) =>
